...
async function loadDirectLessons(ids){
  try{
    ...
    $("lessonDirectCard").innerHTML=`
      <div class="card ok"><b>Scegli una o pi\u00F9 lezioni</b><div class="muted tiny">Puoi selezionare pi\u00F9 disponibilit\u00E0 e prenotarle insieme. Se sei gi\u00E0 iscritto, puoi cancellarti autonomamente solo fino a 48 ore prima.</div></div>
      ${items.map(x=>{
        ...
        return `<div class="card ${left===0?"bad":booked>0?"notice":"ok"}" style="display:flex;gap:12px;align-items:center">
          ...
          <div style="flex:1"><b>${esc(x.title)}</b><div class="muted">${esc(CLUBS[x.clubId]?.name||x.clubId)} \u2022 ${esc(x.date)} \u2022 ${esc(x.time)}\u2013${esc(x.endTime||"")}</div><div class="tiny muted">${booked}/${cap} iscritti \u2022 ${left} posti liberi \u2022 ${price}</div>${myBooking?`<div class="tiny"><b>Sei gi\u00E0 iscritto.</b> ${cancelOk?"Puoi cancellarti entro il limite delle 48 ore.":"Mancano meno di 48 ore: per cancellarti devi contattare il maestro."}</div>`:""}</div>
