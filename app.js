...
async function loadDirectLessons(ids){
  try{
    const [myBookings]=await Promise.all([loadMyBookings()]);
    const items=[];
    for(const id of ids){
    ...
    $("lessonDirectCard").innerHTML=`
      <div class="card ok"><b>Scegli una o piÃ¹ lezioni</b><div class="muted tiny">Puoi selezionare piÃ¹ disponibilitÃ  e prenotarle insieme. Se sei giÃ  iscritto, puoi cancellarti autonomamente solo fino a 48 ore prima.</div></div>
      ${items.map(x=>{
        ...
        return `<div class="card ${left===0?"bad":booked>0?"notice":"ok"}" style="display:flex;gap:12px;align-items:center">
          ...
          <div style="flex:1"><b>${esc(x.title)}</b><div class="muted">${esc(CLUBS[x.clubId]?.name||x.clubId)} â¢ ${esc(x.date)} â¢ ${esc(x.time)}â${esc(x.endTime||"")}</div><div class="tiny muted">${booked}/${cap} iscritti â¢ ${left} posti liberi â¢ ${price}</div>${myBooking?`<div class="tiny"><b>Sei giÃ  iscritto.</b> ${cancelOk?"Puoi cancellarti entro il limite delle 48 ore.":"Mancano meno di 48 ore: per cancellarti devi contattare il maestro."}</div>`:""}</div>
    ...
    await runTransaction(db,async tx=>{
      ...
      activitySnaps.forEach((snap,i)=>{
        ...
        tx.set(bookingRefs[i],{
          ...
          clubId:a.clubId,
          status:"confirmed",
          lessonStart:Timestamp.fromDate(new Date(`${a.date}T${a.time}:00`)),
          cancellationDeadline:Timestamp.fromDate(new Date(new Date(`${a.date}T${a.time}:00`).getTime()-48*60*60*1000)),
          createdAt:serverTimestamp()
        });
...
async function cancelOwnBooking(activityId,bookingId){
  ...
  await removeBooking(activityId,bookingId,S.user.uid,"self");
}
...
async function removeBooking(activityId,bookingId,userId,mode="staff"){
  try{
    await runTransaction(db,async tx=>{
      ...
      if(mode==="self"){
        if(b.userId!==S.user.uid)throw new Error("Non puoi cancellare l'iscrizione di un altro giocatore.");
...
function lessonStaffCard(x,bookings,isAdminView=true){
  ...
  return `<div class="card ${left===0?"bad":booked>0?"notice":"ok"}" style="display:block">
    ...
    <div style="margin-top:10px;border-top:1px solid #173950;padding-top:8px">
      ...
      ${bookings.length
        ? `<div style="display:flex;flex-direction:column;gap:7px">${bookings.map(b=>`<div style="display:flex;gap:7px;align-items:center;flex-wrap:wrap"><button class="btn" ${isAdminView?`data-player-id="${b.userId}"`:""} style="padding:7px 9px">${esc(b.userName||b.userEmail||"Giocatore")}${b.playerLevel?` Â· ${esc(b.playerLevel)}`:""}</button><button class="danger" data-remove-booking="${b.id}" data-remove-activity="${x.id}" data-remove-user="${b.userId}" style="padding:7px 9px">Rimuovi</button></div>`).join("")}</div>`
...
async function adminLessons(){
  ...
  <div class="card">
    ...
    <button id="shareSelectedLessonsBtn" class="primary" style="width:100%;margin-top:8px">ð² Condividi lezioni selezionate su WhatsApp</button>
